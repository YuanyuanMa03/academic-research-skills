...
def build_zotero_item(paper):
    ...
    if dbcode and dbname and filename:
        item["url"] = (
            f"https://kns.cnki.net/KCMS/detail/detail.aspx?dbcode={dbcode}&dbname={dbname}&filename={filename}"
        )
    elif paper.get("link"):
        item["url"] = paper["link"]
    ...
    if attachments and not already_saved:
        ...
        if files_editable:
            ...
            for att in attachments:
                ...
                print(
                    f"  上传 PDF 到 Zotero ({len(pdf_bytes)} bytes)...", file=sys.stderr
                )
                ...
                if att_status == 201:
                    pdf_results.append(
                        f"  PDF 已附加: {title} ({len(pdf_bytes) // 1024}KB)"
                    )
                else:
                    pdf_results.append(
                        f'  PDF 上传失败: HTTP {att_status} {att_err or ""}'
                    )
