    ...
    for fname in SCHEMA_FILES:
        ...
        try:
            req = urllib.request.Request(
                url, headers={"User-Agent": "CSL-Validator/1.0"}
            )
...
def stage2_schema_validation(tree, verbose=False):
    ...
    if tree is None:
        return {
            ...
            "errors": [
                {
                    "rule": "Schema",
                    "message": "Skipped: XML parse failed in Stage 1",
                    "severity": "error",
                }
            ],
            "skipped": True,
    ...
    if csl_version not in SCHEMA_BRANCHES:
        log_verbose(
            f"Unknown CSL version '{csl_version}', defaulting to 1.0 schema", verbose
        )
        csl_version = "1.0"
...
def _check_r1(root, errors, verbose):
    ...
    if not style_class:
        errors.append(
            {
                "rule": "R1",
                "message": "<style> missing 'class' attribute",
                "severity": "error",
            }
        )
    if not style_version:
        errors.append(
            {
                "rule": "R1",
                "message": "<style> missing 'version' attribute",
                "severity": "error",
            }
        )
    ...
    if info is None:
        errors.append(
            {"rule": "R1", "message": "<info> element not found", "severity": "error"}
        )
    else:
        for tag in ("title", "id", "updated"):
            ...
            if el is None or not (el.text and el.text.strip()):
                errors.append(
                    {
                        "rule": "R1",
                        "message": f"<info> missing or empty <{tag}>",
                        "severity": "error",
                    }
                )
    ...
    if citation is None:
        ...
        if not layouts:
            errors.append(
                {
                    "rule": "R1",
                    "message": "<citation> has no <layout> child",
                    "severity": "error",
                }
            )
...
def _check_r6(root, errors, verbose):
    ...
    placeholder_pattern = re.compile(
        r"\[.*?占位.*?\]|\[placeholder\]|\[TODO\]|\[FIXME\]|\[TBD\]", re.IGNORECASE
    )
...
def validate_csl(filepath, verbose=False):
    ...
    for s in stages:
        if s.get("skipped"):
            ...
            real_warnings.extend(
                e for e in s.get("errors", []) if e["severity"] == "warning"
            )
        else:
            real_warnings.extend(
                e for e in s.get("errors", []) if e["severity"] == "warning"
...
def main():
    ...
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Show detailed progress on stderr"
    )
