...
def validate(data: dict) -> list[Finding]:
    ...
    for claim in claims:
        ...
        if not text.strip():
            add(
                findings, "ERROR", "EMPTY_CLAIM", f"权利要求{claim.get('number')}为空。"
            )
        ...
        for source_id in referenced:
            if source_ids and source_id not in source_ids:
                add(
                    ...
                    "ERROR",
                    "UNKNOWN_SOURCE_ID",
                    f"{ledger_id}引用未知来源ID：{source_id}。",
                )
    ...
    for item in formula_inventory:
        ...
        if source_ids and source_id not in source_ids:
            add(
                findings,
                "ERROR",
                "FORMULA_INVENTORY_SOURCE",
                f"公式清单引用未知来源ID：{source_id}。",
            )
        if not item.get("disposition"):
            add(
                ...
                "ERROR",
                "FORMULA_DISPOSITION",
                f"来源公式{source_id}缺少处理去向。",
            )
    expected_formula_count = source_analysis.get("formula_count_in_source")
    if isinstance(expected_formula_count, int) and expected_formula_count != len(
        formula_inventory
    ):
        add(
            findings,
        ...
        for source_id in equation.get("source_ids", []):
            if source_ids and source_id not in source_ids:
                add(
                    ...
                    "ERROR",
                    "EQUATION_SOURCE",
                    f"公式{number}引用未知来源ID：{source_id}。",
                )
    ...
    for item in data.get("figure_inventory", []):
        ...
        if source_ids and source_id not in source_ids:
            add(
                findings,
                "ERROR",
                "FIGURE_INVENTORY_SOURCE",
                f"附图清单引用未知来源ID：{source_id}。",
            )
        if not item.get("disposition"):
            add(
                findings,
    ...
    for figure in figures:
        if not figure.get("source_ids"):
            add(
                ...
                "WARNING",
                "FIGURE_SOURCE",
                f"图{figure.get('number')}缺少来源ID或重绘依据。",
            )
        ...
        for node in figure.get("nodes", []):
            if str(node.get("id")) in end_nodes and VAGUE_RESULT.search(
                str(node.get("label", ""))
            ):
                add(
                    findings,
    ...
    for dimension, threshold in QUALITY_THRESHOLDS.items():
        ...
        if score < 1 or score > 5:
            add(
                findings, "ERROR", "QUALITY_RANGE", f"{dimension}评分超出1-5：{score}。"
            )
    ...
    if source_analysis.get("contains_core_formulas"):
        ...
        if formula_item.get("score", 0) < 4:
            add(
                ...
                "ERROR",
                "FORMULA_SCORE",
                "存在核心公式时，formula_coverage必须至少为4。",
            )
    if figures:
...
def main() -> int:
    ...
    parser.add_argument(
        "--report", type=Path, help="Write the validation report to a file"
    )
    ...
    if args.json:
        print(
            json.dumps(
                [item.__dict__ for item in findings], ensure_ascii=False, indent=2
            )
        )
    else:
        print(report, end="")
