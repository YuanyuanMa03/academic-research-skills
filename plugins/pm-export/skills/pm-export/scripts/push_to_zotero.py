...
def parse_pubmed_authors(author_str):
    ...
    for name in re.split(r",\s*", author_str):
        ...
        if len(parts) == 2:
            authors.append(
                {"lastName": parts[0], "firstName": parts[1], "creatorType": "author"}
            )
        else:
...
def save_attachment(
    session_id, parent_item_id, pdf_bytes, pdf_url, title="Full Text PDF"
):
...
def main():
    ...
    uri = (
        f'https://pubmed.ncbi.nlm.nih.gov/{papers[0].get("pmid", "")}/'
        if papers[0].get("pmid")
        else ""
    )
